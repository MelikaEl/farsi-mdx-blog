...
//this code convert the date of posts in blog page to persian date
// cachePosts.js in the utils folder
import fs from "fs";
import path from "path";
import matter from "gray-matter";
...
const persian = DateObject.calendars.persian;
const persian_fa = DateObject.locales.persian_fa;
...
export function getPosts(
 ...
 /* posts.forEach((post) => {
  ...
  posts.forEach((post) => {
    ...
    post.formattedDate = `${persianDate.format("DD MMMM YYYY")}`;//YYYY-MM-DD
  });
