...
const Editor: React.FC<EditorProps> = ({ initialContent, onContentChange }) => {
  ...
  const editor = useEditor({
    extensions: [Document, Paragraph, Text, Bold, Italic,Heading.configure({
      levels: [1, 2, 3],
    }),],
...
interface EditorProps {
  ...
  const Editor: React.FC<EditorProps> = ({ initialContent, onContentChange }) => {
    ...
    `
        ...
        return (
          <EditorProvider slotBefore={<MenuBar />} extensions={extensions} content={content} onChange={debouncedContentUpdate}></EditorProvider>
        )
}
